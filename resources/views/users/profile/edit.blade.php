        ...
        <div class="col-8">
            <form action="{{ route('profile.update') }}" method="post" class="bg-white shadow rounded-3 p-5" enctype="multipart/form-data">
                ...
                <div class="row mb-3">
                    <div class="col-4">
                        @if ($user->avatar)
                            <img src="{{ $user->avatar }}" alt="{{ $user->name }}" class="img-thumbnail rounded-circle d-block mx-auto avatag-lg">
                        @else
                ...
                <div class="mb-3">
                    ...
                    <textarea name="introduction" id="introduction" rows="5" class="form-control" value="{{ old('introduction', $user->introduction) }}" placeholder="Descrobe yourself">{{ old('introduction', $user->introduction) }}</textarea>
                    @error('introduction')
                        <div class="text-warning small">{{ $message }}</div>
                    @enderror
                </div>
